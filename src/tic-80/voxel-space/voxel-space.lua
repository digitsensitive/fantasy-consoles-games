...
function draw()
    ...
    renderAlgorithm(point:new(camera.p.x,camera.p.y), camera.phi, camera.height, camera.horizon, camera.scaleHeight, camera.distance)
end
...
function renderAlgorithm(p, phi, height, horizon, scaleHeight, distance)
    ...
    for z = distance,1,-1 do
        ...
        for x = 0,GS.W,1 do
            ...
            local heightOnScreen = ((height - heightMap[floorNumber(leftPoint.y)][floorNumber(leftPoint.x)]) / z * scaleHeight) + horizon
