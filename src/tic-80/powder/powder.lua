...
function swapParticle(x1, y1, x2, y2)
	particles[y1 * GS.W + x1].prop, particles[y2 * GS.W + x2].prop =
		particles[y2 * GS.W + x2].prop, particles[y1 * GS.W + x1].prop
end
...
function applyFallAndSink(x, y)
	...
	if lowerParticle.prop.type ~= P_TYPE.AIR then
		if GS.t % 2 == 0 then
			if
				lowerLeftParticle.prop.type == P_TYPE.AIR
				or lowerLeftParticle.prop.sinkable and leftParticle.prop.type == P_TYPE.AIR
				or leftParticle.prop.sinkable
			then
				swapParticle(x, y, x - 1, y + 1)
			end
		else
			if
				lowerRightParticle.prop.type == P_TYPE.AIR
				or lowerRightParticle.prop.sinkable and rightParticle.prop.type == P_TYPE.AIR
				or rightParticle.prop.sinkable
			then
				swapParticle(x, y, x + 1, y + 1)
			end
		end
